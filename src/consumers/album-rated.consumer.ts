...
export async function startAlbumRatedConsumer() {
  ...
  await channel.consume(queue, async (message) => {
    ...
    try {
      ...
      console.log(`ALBUM_RATED received for feed: albumId=${event.albumId}, userId=${event.userId}`);

      await feedService.handleAlbumRated(event);
      channel.ack(message);
    } catch (error) {
      console.error(error instanceof Error ? error.message : 'Unexpected feed consumer error.');
